...
to test_sens
  ...
  ask animals with [ status = "Infected"][

   let a random-float 1.0

   ifelse a <= test_sensitivity[

      set c_slaughtered c_slaughtered + 1
      die]
    [
      set c_FP c_FP + 1
...
to test_spec
  ...
  let spec_animals animals with [status = "Susceptible" or status = "Exposed"]

  ;show count spec_animals
...
to sus_to_exp_A
  ...
  ask n-of floor(inf_rate_D) diarys[die]
